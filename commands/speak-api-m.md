---
description: Summarize the previous response and speak it aloud via ElevenLabs v3 with expressive audio tags. Length auto-scales to the response, or force it with --brief/--medium/--detailed. An optional personality argument shapes the wording and tag choice; defaults to "Q from James Bond".
argument-hint: [--brief|--medium|--detailed] [personality — empty defaults to Q from James Bond]
allowed-tools: Write, Bash
---

# /speak-api-m — ElevenLabs v3 narration
...
- **Personality** — whatever text remains after removing any flag. If that is empty, default to **a young, intelligent, professional British male — think "Q" from James Bond**: crisp received-pronunciation diction, dry wit, understated confidence, brisk-but-measured rhythm, occasional clipped Britishisms like *right then*, *rather*, *do call*, *indeed*.
...
Recall your **most recent assistant message** in this conversation (the turn immediately before `/speak-api-m` was invoked). Recast it as conversational spoken text **in the voice of the personality**. **How long it runs is set by the length tier** resolved from the arguments:
...
If there is no prior assistant message (this is the first turn), reply once with `Nothing to speak yet — invoke /speak-api-m after I've responded.` and stop.
...
**"laid back and friendly girl":**

> [cheerful][relaxed] Okay so — I just finished wiring everything up for you. [chuckles] [short pause] Honestly? [content] Went way cleaner than I thought… all the pieces are talking now. [warmly] Holler if you wanna tweak anything.
...
After `afplay` returns, reply with **one short line** confirming playback and naming the personality voice **and the length tier** used (e.g. `Spoken as: Q from James Bond · auto→medium.` or `Spoken as: gruff sailor · brief.`). Do **not** re-display the summary or the tagged text — the user heard it.
